		...
		else
		{
			...
			if( pxRLUBuffer != NULL )
			{
				...
				if( pxRLUBuffer->bModified == pdTRUE )
				{
					...
					if( lRetVal < 0 )
					{
						FF_PRINTF( "FF_GetBuffer(): FF_BlockWrite() failed\n");
				...
				else
				{
					...
					if( lRetVal != FF_ERR_NONE )
					//if( lRetVal < 0 )
					{
						FF_PRINTF( "FF_GetBuffer(): FF_BlockRead() failed at %#08x\n",ulSector);
...
int32_t slRetVal = 0;
	...
	if( pxIOManager->xPartition.ulTotalSectors != 0ul )
	{
		...
		{
			FF_PRINTF( "FF_BlockRead(): ERROR OutOfBounds\n");

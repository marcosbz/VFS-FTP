...
FF_CreationParameters_t xParameters;
	...
	if( pxDisk != NULL )
	{
		...
		if( ( pxDisk->pxIOManager != NULL ) && ( FF_isERR( xError ) == pdFALSE ) )
		{
			...
			FF_PRINTF( "FF_RAMDiskInit: RAMDisk initialized\n");
...
uint8_t *pucSource;
	...
	if( pxDisk != NULL )
	{
		...
		{
			FF_PRINTF( "prvReadRAM(): MagicNumber\n");
		...
		{
			//FF_PRINTF( "prvReadRAM(): NoInit\n");
			//FF_PRINTF( "prvReadRAM(): Disk not init at %#08x\n",ulSectorNumber);
	...
	{
		FF_PRINTF( "prvReadRAM(): ERROR NULLPointer\n");

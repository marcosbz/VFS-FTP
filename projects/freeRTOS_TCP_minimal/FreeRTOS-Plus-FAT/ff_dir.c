...
FF_Error_t xError;
	...
	if( pxIOManager == NULL )
	{
		...
		printf("FF_FindFirst(): Error Null Manager\n");
	}
	...
	else if( ( pxIOManager->ucFlags & FF_IOMAN_DEVICE_IS_EXTRACTED ) != 0 )
	{
		...
		printf("FF_FindFirst(): Error NoMedium\n");
	}
	...
	else
	{
		...
		if( FF_isERR( xError ) == pdFALSE )
		{
			if( pxDirEntry->ulDirCluster == 0 )
			{
				...
				printf("FF_FindFirst(): Error DirInvalidPath\n");
			}
			else
			{
				...
				{
					printf("FF_FindFirst(): Error in FF_InitEntryFetch()\n");
				}
			}
		}
	}

...
const char *pcDirectory;
	...
	if( iReturn == 0 )
	{
		...
		if(iReturn)
		{
			FF_PRINTF("ff_findnext() returns error %i\n",iReturn);
		}
	}
	else
	{
		/* errno has already been set. */
		FF_PRINTF("ff_findfirst() returns error %i\n",iReturn);
	}
	...
	else
	{
		...
		if( pxFindData->xDirectoryHandler.pxManager != NULL )
		{
			if( pxFindData->xDirectoryHandler.u.bits.bFirstCalled == pdFALSE )
			{
				...
				if(xError)
				{
					FF_PRINTF("ff_findnext: FF_FindFirst returns error\n" );
				}
			}
			else if( pxFindData->xDirectoryHandler.u.bits.bEndOfDir == pdFALSE )
			{
				...
				if(xError)
				{
					FF_PRINTF("ff_findnext: FF_FindNext returns error\n" );
				}
			}
			...
			if( FF_GETERROR( xError ) == FF_ERR_DIR_END_OF_DIR )
			{
				...
				FF_PRINTF("ff_findnext: End of dir\n" );
			}

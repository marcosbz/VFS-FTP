...
void USB_Host_ProcessNextHostState(uint8_t corenum)
{
	...
	else if (USB_HostState[corenum] == HOST_STATE_Unattached) /* CUSTOM: Added to detect previously attached device */
	{
		if(HCD_STATUS_OK == HcdGetDeviceConnectStatus(corenum))
			USB_Host_Enumerate(corenum);
	}
}
...
HCD_STATUS USB_Host_GetDeviceConnStatus(uint8_t hostid)
{
	if (hostid <= 1) {
		return HcdGetDeviceConnectStatus(hostid);
	}
	else {return HCD_STATUS_DEVICE_DISCONNECTED; }
}
